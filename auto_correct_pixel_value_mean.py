...
print("\nmean pixel value ( Input image (LR = 1) )\n>", mean_LR1, "(pixel value)")
...
p = p_init
tmp_mean = 0
#tmp_img_Gray = cv2.cvtColor(img_in_RGB, cv2.COLOR_RGB2GRAY)
while tmp_mean < mean_LR1:
    ...
    #tmp_corrected_img_Gray = cv2.multiply(tmp_img_Gray, p)

    # Calc mean pixel value
    tmp_corrected_img_Gray_nonzero = tmp_corrected_img_Gray[tmp_corrected_img_Gray > 0]
    tmp_mean = tmp_corrected_img_Gray_nonzero.mean()
...
p_final = round(p, 2)
mean_final = round(tmp_mean, 1)
print("p_final\n>", p_final)
print("\nmean pixel value (Input image)\n>", mean_final, "(pixel value)")
count_overexpose = np.sum(tmp_corrected_img_Gray == 255)
...
# ----------------------------------
fig_name = "images/figure_"+str(p_final)+"_"+str(mean_final)+".png"
...
output_img_name = "images/improved_"+str(p_final)+"_"+str(mean_final)+".jpg"
